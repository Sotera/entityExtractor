...
var app = require('../server/server'),
  ...
  mitieUrl = 'http://mitie:9990/', // dev tip: add mitie to /etc/hosts
  polyglotUrl = 'http://172.17.0.1:5000/', // dev tip: add polyglot to /etc/hosts
...
module.exports = {
  ...
  // TODO: extract text by mimetype, see mitie below.
  extractWithPolyglot(text, mimeType) {
    //mimeType = mimeType || 'text/html';
    var context = this;
    return syncPromises(function* () {
      return context.requestPolyglot(text);
    })();
  },

  requestPolyglot(text) {
    var client = requestJson.newClient(polyglotUrl);
    var cleanText = text.replace(/\n/g, ",", ' ');
    return new Promise((res, rej) => {
      client.post('extract', { text: cleanText }, (err, _, body) => {
        if (err) {
          console.error(err);
          return rej(err);
        }
